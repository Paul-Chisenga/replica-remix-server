...
const Shop = () => {
  ...
  const { mcat, m, mt, s } = Object.fromEntries(searchParams);
  ...
  return (
    <>
      <Breadcrumb pageName="Shop" pageTitle="Menu" />
        ...
        <div className="container">
          <div className="row g-lg-5 gy-5">
                ...
                <div className="single-widgets widget_egns_tag">
                  ...
                  <ul className="wp-block-tag-cloud">
                    {menuList.map((item) => (
                      <li key={item.id}>
                        <Link
                          to={`?m=${item.id}&mt=${item.title}#products`}
            ...
            <div
              ...
              id="products"
              // style={{ paddingTop: location.hash ? "128px" : "0" }}
            >
              <div>
                <div className={`tw-text-xl tw-text-black mb-30`}>
                  {count} results found
                  {(s || mt || mcat) && (
                    <span className="tw-inline-block tw-font-bold">
                      {" "}
                      : {s ?? mt ?? parseMenuCategory(mcat as any)}
                    </span>
                  )}
                </div>
              </div>
  ...
  try {
    const productWhere: Prisma.ProductWhereInput = {
      OR: [
        ...
        { title: !s && !m && !mcat ? { contains: "" } : undefined },
      ],
    };
