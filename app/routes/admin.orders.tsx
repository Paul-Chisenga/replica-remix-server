...
export default function ComponentName() {
  ...
  const orders = {
    new: loaderData.filter((order) => order.status === OrderStatus.recieved),
    preparing: loaderData.filter(
      (order) => order.status === OrderStatus.preparing
    ),
    shipping: loaderData.filter(
  ...
  return (
    <div className="tw-p-4">
      ...
      <Tab.Wrapper>
        <Tab.Header.Wrapper id="orderTabHeader">
          {Object.entries(orders).map(([key, items], idx) => (
            <Tab.Header.Item key={key} id={key} defaultSelected={idx === 0}>
              {key + ` (${items.length}) `}
            </Tab.Header.Item>
          ))}
        </Tab.Header.Wrapper>
        <Tab.Content.Wrapper id="orderContent">
          {Object.entries(orders).map(([key, selectedOrders], idx) => (
            <Tab.Content.Item key={key} id={key} defaultSelected={idx === 0}>
                ...
                <table className="my-table admin-orders-table table">
                  ...
                  <tbody>
                    {selectedOrders.length === 0 && (
                      <tr className="tw-p-5 tw-bg-white tw-rounded-xl tw-overflow-hidden tw-border-gray-100">
                      ...
                      return (
                        ...
                        >
                          ...
                          <td data-label="Customer Details" className="">
                            ...
                            <p>{order.customer.profile.email}</p>
                            <p>0{order.customer.profile.phone}</p>
                          </td>
